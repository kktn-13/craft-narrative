            ...
            <div key={status} className="flex flex-col items-center flex-1">
                ...
                {index > 0 && (
                  <div
                    className={`absolute left-0 right-1/2 h-0.5 top-4 -translate-y-1/2 ${
                      isCompleted ? "bg-craft-600" : "bg-craft-200"
                    }`}
                  />
                )}
                {index < STATUSES.length - 1 && (
                  <div
                    className={`absolute left-1/2 right-0 h-0.5 top-4 -translate-y-1/2 ${
                      index < currentIndex ? "bg-craft-600" : "bg-craft-200"
                    }`}
                  />
                )}
                  ...
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
                      isCompleted
                        ...
                        : "bg-craft-100 text-craft-400"
                    } ${isCurrent ? "ring-2 ring-craft-300 ring-offset-2" : ""}`}
                  >
                    {isCompleted ? (
              ...
              <p
                className={`text-xs mt-2 ${
                  isCurrent ? "font-semibold text-craft-800" : "text-ink-400"
                }`}
              >
                {t(status)}
              </p>
        ...
        <div className="bg-craft-50 border border-craft-200 rounded-lg p-4">
          ...
          {carrier && (
            <p className="text-sm text-ink-600">
              <span className="text-ink-400">{tDash("carrier")}:</span>{" "}
              {carrier}
            </p>
          )}
          {trackingNumber && (
            <p className="text-sm text-ink-600 mt-1">
              ...
              <code className="bg-white px-2 py-0.5 rounded text-craft-800">
                {trackingNumber}
              </code>
            </p>
          )}
